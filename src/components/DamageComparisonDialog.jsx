...
import { DAMAGE_COMPARISON_FILTERS } from "../domain/skill-damage-ranking.js";
...
export default function DamageComparisonDialog({ snapshot, source, preferences, onPreferencesChange, onClose, onImport }) {
  ...
  return <div className="damage-comparison-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={dialog} className="damage-comparison" role="dialog" aria-modal="true" aria-label="承伤对比" onKeyDown={onKeyDown}>
      ...
      <div className={`dc-web-options${model.filtersOpen ? " is-open" : ""}`}>
      ...
      <div className="dc-web-filters" role="group" aria-label="承伤筛选">{DAMAGE_COMPARISON_FILTERS.map(([key, text]) => <button type="button" key={key} aria-pressed={model.filter === key} onClick={() => model.setFilter(key)}>{text}</button>)}<label className="dc-web-inherit"><input type="checkbox" checked={model.inheritTargetStatuses} onChange={(event) => model.setInheritTargetStatuses(event.target.checked)} />沿用星陨／冻结</label></div>
      <div className="dc-web-summary"><span role="status">{model.loading ? `正在计算 ${model.progress?.completed ?? 0}/${snapshot.spirits.length}` : `${model.rows.length} 只 · ${model.template.label}`}</span><button type="button" aria-expanded={model.showExcluded} onClick={() => model.setShowExcluded(!model.showExcluded)}>未纳入 {model.ranking?.excluded.length ?? 0}</button></div>
      <div className="dc-web-scroll" aria-busy={model.loading}>
        {model.error || model.ranking?.issue ? <p className="dc-web-empty" role="status">{model.error || model.ranking.issue}</p> : null}
        {model.showExcluded ? <section className="dc-web-excluded" aria-label="未纳入原因"><h3>未纳入原因</h3>{model.ranking?.excluded.map(({ spirit, reason }) => <p key={spirit.id}><span>{spirit.fullName}</span><small>{reason}</small></p>)}</section> : null}
        {model.rows.length > 0 ? <div className="dc-web-columns" aria-hidden="true"><span>排名</span><span>精灵</span><span>伤害 HP</span><span>承伤比例</span></div> : null}
        {model.rows.slice(0, model.limit).map((row) => {
          ...
          return <div key={row.spirit.id} className={`dc-web-item${expanded ? " is-expanded" : ""}`}>
            ...
            {expanded ? <div className="dc-web-detail" id={`damage-detail-${row.spirit.id}`}>
              <div><strong>{row.spirit.fullName}</strong><p>生命 {row.panelStats.hp} · 物防 {row.panelStats.physicalDefense} · 魔防 {row.panelStats.magicalDefense}</p><p>本次伤害 {row.damage} · 剩余 {row.remainingHp} HP</p><small>{model.templateDescription(row)}<br />本榜不计防守方特性，代入后恢复特性。</small></div>
        ...
        {model.rows.length > model.limit ? <button type="button" className="dc-web-more" onClick={model.showMore}>继续显示（剩余 {model.rows.length - model.limit} 只）</button> : null}
      </div>
