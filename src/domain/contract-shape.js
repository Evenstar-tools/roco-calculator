...
export const CONTRACT_BALLS = Object.freeze([
  { value: "normal", label: "普通球", summary: "攻防速 +10%" },
  { value: "advanced", label: "高级球", summary: "攻防速 +20%" },
  { value: "king", label: "国王球", summary: "攻防速 +30%" },
  { value: "beautiful", label: "美妙球", summary: "对方双攻 -30% · 威力 +20" },
  { value: "temperature", label: "调温球", summary: "对方灼烧 ×4 · 冻结 ×2" },
  { value: "photosynthesis", label: "光合球", summary: "能耗 -1 · 连击 +1" },
  { value: "net", label: "网兜球", summary: "速度 +50 · 对方中毒 ×2" },
  { value: "insulation", label: "绝缘球", summary: "对方速度 -40 · 物防 -40% · 连击 -2" },
  { value: "sand", label: "淘沙球", summary: "双防 +30% · 对方星陨 ×1" },
  { value: "transform", label: "变幻球", summary: "吸血 +30% · 对方能量 -2" },
  { value: "darkstar", label: "暗星球", summary: "物攻 +40% · 对方魔防 -40%" },
...
export function resolveContractShape({
  ...
  switch (effectiveBallType) {
    ...
    case "advanced":
    case "king": {
      const level = { normal: 1, advanced: 2, king: 3 }[effectiveBallType] * scale;
    ...
    case "temperature":
      result.settlement = settlement({
        ...
        ownerRole,
        status: "recorded",
        text: `${prefix}｜对方灼烧 ×${scaledDiscrete(4, scale)}、冻结 ×${scaledDiscrete(2, scale)} · 本次伤害不追加`,
      });
      break;
    case "photosynthesis":
      ...
      result.settlement = settlement({
        ...
        effectiveEntry,
        ownerRole,
        text: `${prefix}｜能耗 -${scaledDiscrete(1, scale)} · 连击 +${result.hitCountAdd}${scale < 1 ? " · 离散值向零取整" : ""}`,
      });
      break;
    case "net":
      result.ownerSpeedFlat = 50 * scale;
      result.targetPoisonStacksAdd = scaledDiscrete(2, scale);
      result.settlement = settlement({
        ballType,
        effectiveEntry,
        ownerRole,
    ...
    case "insulation":
      ...
      result.targetHitCountAdd = hasDeclaredHitCount(skill)
      ...
      result.settlement = settlement({
        ...
        effectiveEntry,
        ownerRole,
        text: `${prefix}｜对方速度 -${40 * scale} · 物防 -${40 * scale}% · 连击 ${result.targetHitCountAdd}`,
      });
      break;
    case "sand":
    ...
    case "transform":
      result.settlement = settlement({
        ...
        ownerRole,
        status: "recorded",
        text: `${prefix}｜吸血 +${30 * scale}% · 对方能量 -${scaledDiscrete(2, scale)} · 仅记录`,
      });
      break;
    case "darkstar":
