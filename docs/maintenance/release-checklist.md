...
2026-09-20 按用户“适合未来持续更新，不要反复处理”的要求重设长期预算：总资源预警 20 MiB／硬上限 24 MiB，JS gzip 400／512 KiB，CSS gzip 72／96 KiB，原始 JS 1.5／2 MiB，运行数据 2／2.5 MiB。以 `scripts/verify-performance-budget.mjs` 为执行来源；详细口径与依据见 [长期体积预算](performance-budget-policy.md)。超过预警只提示并登记优化，不因零碎增长反复询问；超过硬上限停止发布并优先定位重复依赖或拆分加载，不自动扩容。小程序主包平台限制仍为 2 MiB，本次不改。

- [ ] 日常测试包含预算边界与归因自测，防止脚本、测试和文档的阈值再次不一致。
- [ ] 日常 E2E 包含既有冷启动、热启动、技能搜索耗时检查（10s／5s／1.5s），仅为本地回归门禁，不能作为移动网络或真实用户性能达标的证明。
