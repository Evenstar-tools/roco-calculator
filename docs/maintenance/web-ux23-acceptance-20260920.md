...
用户在上线后明确要求：已有性格时默认展开所属栏位，例如胆小展开速度增益。按用户两张图调整 W23-01 的初始展开规则，替代之前“始终首开七行”的约定；不修改布局、字号、颜色或悬停切组行为。

每次打开从当前性格读取 upStat，仅展开对应一组五项，总十二行；选普通无修正时仍七行。只浏览其他组再关闭，重新打开仍定位实际已选组，打开本身不调用 onChange、不添加撤回记录。

验证：先新增回归用例，旧实现 31 项失败；修正后性格菜单与快捷键专项 57 项全部通过，覆盖全部 30 个增益性格、无修正、重开与悬停切组；定向 lint、diff 检查、生产构建与预算校验通过。JS gzip 350.67 KiB、CSS gzip 56.57 KiB，预算不变。

真实 Web 实测胆小自动展开速度、浏览生命后重开仍回速度、无修正回七行；对照 `artifacts/web-ux23/default-group-acceptance/preview.html`。首次取图因重载后未重设精灵导致控件不存在，补齐独立测试页面数据后完成截图及回读。本追加只在本地实现，未提交推送，未单独重测 Electron 运行时。

## 追加 W23-04：下注·暗触发入口

用户截图明确指定“下注·暗”名称下方增加与“通电”相同的勾选控件。沿用 `deer-check deer-electric`，没有新增 CSS 或弹窗。文字为“触发暗特效”；勾选把当前页自身HP设为49%，取消设为100%。复用 setOptions、整数生命构建与共享伤害内核，伤害、斩杀层数及条件摘要同步变化，不改目标HP、通电状态、预设或收藏。

勾选状态由当前实际整数生命是否严格低于最大生命一半推导，不保存独立布尔开关；手动修改血量或更换自身配置后同步。默认377最大生命时，49.99%取整为188，低于188.5，仍应勾选并有185威力；50%取整189则不触发。首次新增边界测试误把49.99%写成不触发，经实际生命与共享内核读回纠正测试，未改计算规则。

本地验证：电鹿页面、关键交互和领域专项3文件／75项通过；定向 lint、git diff 检查、生产构建及体积门禁通过，JS gzip 350.80 KiB、CSS gzip 56.57 KiB，不调整预算。真实 Web 1280×1100亮暗、390/320×844暗色验收；勾选、取消、20%与50%手改双向联动，目标仍100%，通电不被勾选，窄屏无横向溢出。

证据：`artifacts/web-ux23/deer-dark-acceptance/preview.html` 及实际合图；`verify-deer-dark.js` 可重跑。已读回亮色和320px实际截图及合图。仅本地实现，保留上一项性格自动展开的未提交改动；本次未提交、推送或重测桌面安装包。
