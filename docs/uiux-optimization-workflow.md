...
1. 用户指定的平台和尺寸优先。当前重点为 Web 横屏 1920×1080；窄屏仅做兼容检查，不主动扩大小程序范围。未指定平台的跨平台轮次才先小程序、后 Web/桌面。
2. 每个平台为一个独立批次:实施 → 验收 → 单独提交,不跨平台混提交。
3. 一轮结束后如用户说"再跑一轮",先读 `docs/maintenance/web-ux-backlog.md`，从未完成项中按价值和风险挑选最多 3 项，轮次编号 +1。候选尚有余量时不重复全站巡检；不足 3 项不凑数，待办耗尽或证据已失效后才补巡检。

## 待办池维护（每轮先读、结束必更新）

- 所有发现都入池，包括未入选、暂缓、疑似问题和已排除项；记录稳定编号、发现日期、视口/主题/模式、基线提交、复现与证据、建议、优先级和状态。
- 本轮只挑最多 3 项进入对图确认；未选项保留，不遗忘、不重复创建。已完成项记录提交及验收，已否决项记录理由；相同问题合并到原编号。
- 已有候选只做针对性的复现检查；若相关组件已变更或原证据不能复现，标记待复核后再检查，不把旧截图当当前事实。
- 减少的是重复发现，不是实施后的验收。每次落地仍执行相关功能测试、视觉读回和发布门禁。
- 视觉改动仍遵守下面的用户确认门槛；“入选”不等于“已实施”。
...
1. 先复用待办池。仅在需要补充候选时，用 Playwright 对指定平台做截图巡检；当前以 Web 1920×1080 的双方配置、四技能、条件条、结果栏、菜单和弹层为主，320/390px 仅复核受影响区域的兼容性。记录实际覆盖范围，不宣称未检查区域已验收。
2. 所有发现记入待办池，只给本轮入选的最多 3 项做 **before/after 对比图**:
