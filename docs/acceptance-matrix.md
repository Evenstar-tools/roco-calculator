...
| 离线 Web 与 Windows 桌面包 | 通过 | `public/sw.js`、`desktop/main.mjs`、`scripts/build-desktop.mjs` | `tests/e2e/offline-performance.spec.js`、`tests/desktop/offline-paths.test.js`、`tests/desktop/desktop-contracts.test.js` | Service Worker 生产断网重载已有自动验证；每次桌面发布仍需执行安装和启动冒烟。 |
