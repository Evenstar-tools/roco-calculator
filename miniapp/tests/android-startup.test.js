...
test.each(["hasOwn"])("缺少 %s 时首页数据、首次计算和撤回仍可用", async (api) => {
  ...
  try {
    ...
    expect(createCalculationView(loaded.snapshot, state)).toHaveProperty("selectedResult");
...
test.each([
  ...
  const output = execFileSync(process.execPath, ["--input-type=module", "-e", `
    ...
    if(snapshot.spirits.length<600 || !view.selectedResult || history.undo().state!==state)throw Error('startup failed');
