...
describe("恶魔男爵逐击吸血实战回归", () => {
  test("开朗 abs 男爵羽化后吃平和 had 骨龙隼鳞，再用撕咬逐击结算", () => {
    ...
    expect(baronStats.hp).toBe(420);
    expect(falconScale.totalDamage).toBe(222);
    expect(input.directions.reverse.currentHp).toBe(198);
    expect(bite).toMatchObject({
      hitCount: 5,
      hitDamages: [151, 151, 151, 151, 166],
      mainDamage: 770,
      postAttackEffects: {
