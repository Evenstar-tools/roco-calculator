...
function createReleaseFixture(localConfig) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ version: "1.6.4" }),
  );
  fs.writeFileSync(
    path.join(root, "miniapp", "package.json"),
    JSON.stringify({ version: "1.1.4" }),
  );
  fs.writeFileSync(
...
describe("miniapp production release gate", () => {
  ...
  test("accepts a complete v1.1.4 production artifact contract", () => {
